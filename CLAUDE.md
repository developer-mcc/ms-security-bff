...
`ms-security-bff` is the **Security BFF** of a multi-tenant pharmacy management system. It owns authentication, JWT issuance/validation, session management, the 3D permission model (role × resource × action × scope), token storage, audit log, rate limiting, and OTP (flujo completo: `POST /auth/otp/request` + `POST /auth/otp/verify`). It backs the PostgreSQL database `poc-security` (schema `sec`).
...
- Spring Boot 4.0.6 + Spring Security 6 + Spring Data JPA + Validation + WebMVC + **Actuator** (`health` + `info` expuestos, `show-details: when-authorized`).
...
- **Resilience4j 2.2.0** (`resilience4j-spring-boot3`) — `@CircuitBreaker` + `@Retry` + `@Bulkhead` en adaptadores de notificación. Spring Mail (`spring-boot-starter-mail`) para `EmailChannelAdapter`.
...
- **OTP — implementado**. `POST /auth/otp/request` (202) + `POST /auth/otp/verify` (204), ambos protegidos con `@PreAuthorize("isAuthenticated()")`. Generación con `SecureRandom` 6 dígitos (`SecureRandomOtpGeneradorAdapter`), hash BCrypt (`BcryptOtpHasherAdapter`), verificación con `@Lock(PESSIMISTIC_WRITE)` para prevenir race conditions, `intentos_maximos=3`, TTL 5 min. Al agotar intentos → `OtpMaxIntentosException` (422); al expirar → `OtpExpiradoException` (422); código incorrecto → `OtpInvalidoException` (422). El incremento de intentos usa `@Transactional(noRollbackFor = {...})` para que sobreviva al throw. `OtpCleanupScheduler` purga cada 15 min con `@Scheduled`. Eventos: `OtpSolicitadoEvent` · `OtpVerificadoEvent` · `OtpFallidoEvent` — auditados por `OtpAuditListener`. La política rate-limit `otp-verify` (3 req / 5 min / USER_ID) ya estaba activa. **Pendiente**: la estrategia de notificación (los canales EMAIL/SMS/WhatsApp aún no envían realmente el código).
...
- **Errors** — RFC 9457 `ProblemDetail` con `traceId` desde MDC. Tres puntos cubren toda la cadena: `GlobalExceptionHandler` (`@RestControllerAdvice`) para excepciones que llegan al controller, `JwtAuthenticationEntryPoint` para 401 y `ForbiddenAccessDeniedHandler` para 403 desde el filter chain. `ProblemDetailWriter` sirve de helper común. **Encoding**: `ProblemDetailWriter` llama a `response.setCharacterEncoding(StandardCharsets.UTF_8.name())` antes de `getWriter()` — los filtros de seguridad corren antes de que Spring Boot's `CharacterEncodingFilter` actúe, así que sin esta línea el Servlet container usa ISO-8859-1 y corrompe tildes/ñ.
- **Zona horaria** — `TimeZoneConfig` (`infrastructure/config/`) es la única fuente de verdad: `ZONE_ID = ZoneId.of("America/Lima")`, `@PostConstruct TimeZone.setDefault(...)` (cubre `LocalDateTime.now()`, `@PrePersist`, librerías de terceros), bean `java.time.Clock` (inyectado en `SystemClockAdapter`), bean `ZoneId` (disponible para conversiones `LocalDateTime ↔ Instant`). Complementado por `spring.jackson.time-zone: America/Lima` (serialización JSON) y `hibernate.jdbc.time_zone: America/Lima` (binding JDBC).
...
| `ProblemDetail` RFC 9457 (handler + entry-point + access-denied-handler con `traceId`) + encoding UTF-8 en filter chain (`response.setCharacterEncoding` en `ProblemDetailWriter`) | ✅ |
...
| **OTP completo**: `POST /auth/otp/request` (202) + `POST /auth/otp/verify` (204) · BCrypt · SecureRandom · PESSIMISTIC_WRITE · 3 intentos · TTL 5 min · `OtpCleanupScheduler` · eventos + auditoría | ✅ |
| Actuator: `/actuator/health` + `/actuator/info` expuestos · `show-details: when-authorized` · `spring-boot-starter-actuator` en `build.gradle` | ✅ |
| Zona horaria `America/Lima`: `TimeZoneConfig` (`@PostConstruct TimeZone.setDefault` + bean `java.time.Clock` + bean `ZoneId`) · `SystemClockAdapter` usa el bean · `spring.jackson.time-zone` · `hibernate.jdbc.time_zone` · `bootRun -Duser.timezone` | ✅ |
| `@Transactional` en todos los métodos `@Modifying` de `TokenJpaRepository` y `OtpJpaRepository` | ✅ |
| `.gitignore` con buenas prácticas (secrets, logs, OS Windows/macOS, JVM crash, temp files) | ✅ |
| Colección Postman `postman/ms-security-bff.postman_collection.json`: carpetas 1–7 (login · refresh/logout · usuarios · perfil · casos negativos · rate-limit · OTP) con assertions UTF-8 | ✅ |
| **Notification Strategy** — `EmailChannelAdapter` (real, JavaMailSender), `SmsChannelAdapter` + `WhatsAppChannelAdapter` (stubs que tiran excepción para forzar fallback a email). `EnviarNotificacionService` selecciona canal, resuelve destinatario, ejecuta con fallback a email. `OtpNotificationListener` (`@TransactionalEventListener(AFTER_COMMIT)` + `@Async`) envía el código OTP tras commit. Resilience4j 2.2.0: `@CircuitBreaker` + `@Retry` en email; `@CircuitBreaker` + `@Bulkhead` en SMS/WhatsApp (sin `@Retry` hasta integrar gateway real). `NotificationResilienceConfig` registra loggers de state-transition. | ✅ |
| **OpenAPI, Tests, Flyway, Virtual Threads, Gradle KTS** | ❌ pendientes |

**Pendientes priorizados** (orden recomendado): P3.3 Flyway → P3.4 OTP en login (2FA) → P3.1 OpenAPI → P3.2 Tests → P4.1 Gradle KTS → P4.2 Virtual Threads.
...
- **`@Transactional` y rollback**: cualquier persistencia que deba sobrevivir al `throw` de un `@Transactional` parent debe ir en un servicio anotado con `@Transactional(propagation = REQUIRES_NEW)`. Patrones aplicados: `RegistrarIntentoFallidoService` (lockout), `RecordAuditService` (bitácora), `VerificarOtpService` (incremento de intentos con `noRollbackFor`). Si añades algo similar, sigue ese patrón.
- **`@Transactional` en métodos `@Modifying`**: todos los métodos `@Modifying` de los repositorios JPA deben llevar `@Transactional` en la propia declaración del repositorio (no solo en el caller). Sin esto, un caller sin transacción activa (como un `@Scheduled`) lanza `InvalidDataAccessApiUsageException: No active transaction`. Patrón aplicado en `TokenJpaRepository` y `OtpJpaRepository`.
...
- **Resilience4j — orden de anotaciones en adaptadores de notificación**: `@Retry` (exterior) → `@CircuitBreaker` → `@Bulkhead` (interior). SMS/WhatsApp stubs usan solo `@CircuitBreaker` + `@Bulkhead` (sin `@Retry`) porque `UnsupportedOperationException` no es un fallo transitorio — agregar `@Retry` + `retryExceptions` cuando se integre el gateway real. `CircuitBreakerRegistry` bean disponible vía autoconfiguration de `resilience4j-spring-boot3`.
- **Notification fallback a email**: `EnviarNotificacionService.enviar(...)` captura cualquier `Exception` del canal primario y reintenta con `CanalOtp.EMAIL`. Si el canal ya era EMAIL y falla, relanza `NotificacionFallidaException`. El `OtpNotificationListener` captura `NotificacionFallidaException` y la loggea (no propaga — el OTP ya está persistido y el usuario puede pedirlo de nuevo).
- **`@TransactionalEventListener(AFTER_COMMIT)` + `@Async` en `OtpNotificationListener`**: `AFTER_COMMIT` garantiza que el código OTP esté en DB antes de enviarlo. `@Async` evita bloquear el hilo de la transacción original. Mismo patrón que `BranchSwitchedAuditListener` y `TokenRevokedAuditListener`.
...
- **Encoding UTF-8 en filtros de seguridad**: los handlers de Spring Security (`AuthenticationEntryPoint`, `AccessDeniedHandler`) escriben directamente en `HttpServletResponse` antes de que Spring Boot's `CharacterEncodingFilter` actúe. Sin `response.setCharacterEncoding(StandardCharsets.UTF_8.name())` antes de `getWriter()`, el container usa ISO-8859-1 y corrompe tildes y ñ. `ProblemDetailWriter` ya lo hace. Cualquier future handler que escriba texto directamente debe replicar este patrón.
- **Zona horaria — fuente única**: `TimeZoneConfig.ZONE_ID` (`ZoneId.of("America/Lima")`) es la constante canónica del proyecto. No hardcodear `"America/Lima"` en ningún otro archivo. El `@PostConstruct` en esa clase garantiza que el JVM default quede fijo antes de cualquier `LocalDateTime.now()`.
